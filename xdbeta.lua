-- [[ ANIMAL HOSPITAL – WINDUI + ANOMALY SENSOR + AUTO HEARTBEAT + INSTANT PROMPTS ]] --
...
local ProximityPromptService = game:GetService("ProximityPromptService")  -- NEW
...
local Window = WindUI:CreateWindow({
    ...
    Size = UDim2.new(0, 550, 0, 480),  -- increased height for new toggle
    Transparent = false,
    Parent = ScreenGui
})
...
MainTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            ...
            CleanupAutoHeartbeat()
        end
    end
})

-- NEW: Instant Prompts Toggle
MainTab:Toggle({
    Title = "Instant Proximity Prompts",
    Desc = "All proximity prompts (treating, picking up) activate instantly",
    Value = false,
    Callback = function(state)
        ...
        if state then
            ...
            if not _G.npcConnection then
                _G.npcConnection = Workspace.NPCs.ChildAdded:Connect(function(instance)
                    if Cheats.AnomalySensor then
                        HighlightNPC(instance)
                    end
                end)
            end
        else
...
local function SetupAutoHeartbeat()
    ...
    if not Net then
        warn("Auto Heartbeat: 'Net' folder not found")
        return
    end
...
LoaderTab:Paragraph({
    ...
    Content = "Paste raw Lua script code from Pastebin or other sources below and click 'Execute Script' to run it."
})
...
LoaderTab:Button({
    ...
    Callback = function()
        ...
        if code and code ~= "" then
            ...
            if success then
                print("Pasted script executed successfully!")
            else
                warn("Failed to execute pasted script: " .. tostring(err))
            end
        else
...
LoaderTab:Button({
    ...
    Callback = function()
        ...
        if url and url ~= "" then
            ...
            if success then
                print("Script loaded from URL successfully!")
            else
                warn("Failed to load from URL: " .. tostring(err))
            end
        else
...
RunService.Stepped:Connect(function()
    ...
    if Cheats.InfiniteSanity then
        ...
        if sanity then
            if sanity:IsA("NumberValue") then
                sanity.Value = 100
            elseif sanity:IsA("IntValue") then
                sanity.Value = 100
            end
        end
    ...
    -- FullBright (handled by toggle callback)
end)
